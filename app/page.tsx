...
import {
  Search, X, ShoppingCart, MessageCircle, Store, Sparkles,
...
export default function Home() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const saved = localStorage.getItem("vidriera-cart");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("vidriera-cart", JSON.stringify(cart));
    } catch (e) {}
  }, [cart]);
  ...
  const font = FONT_OPTIONS.find((f) => f.id === settings?.font_id) || FONT_OPTIONS[0];
  const primary = settings?.primary_color || "#1F6F6F";
  ...
  function checkout() {
    ...
    const msg = `Hola ${settings?.name}! Quiero hacer este pedido:\n\n${lines.join("\n")}\n\nTotal: $${fmt(total)}`;
    ...
    <div style={{ fontFamily: font.body }}>
      <div className="px-6 py-12 text-center" style={{ background: `${primary}14` }}>
        ...
        <h1 className="text-3xl font-semibold" style={{ fontFamily: font.heading }}>
          {settings?.name || "Mi Emprendimiento"}
        </h1>
        <p className="text-sm text-neutral-500 mt-1 max-w-md mx-auto">{settings?.description}</p>
      </div>
      ...
      {products.length === 0 ? (
        <div className="text-center py-20 text-neutral-400 px-6">
          <Store size={28} className="mx-auto mb-3 opacity-40" />
